  ...
  app.post(
    ...
    async (req: any, res: any) => {
      try {
        ...
        const reportData = await reportService.getExpenseReport({});
        const budgetData = await budgetService.getAllBudgets();
        const requireGemini = req.body?.requireGemini === true;
        const assistantResponse = requireGemini
          ? await aiAnalysisService.askAssistantWithGemini(question, reportData, budgetData, { requireGemini: true })
          : await aiAnalysisService.askAssistantWithGemini(question, reportData, budgetData);
