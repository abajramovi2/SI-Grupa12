...
`;
  ...
  async askAssistantWithGemini(
    question: string,
    reportData: any,
    budgetData: any,
    options: { requireGemini?: boolean } = {}
  ): Promise<{ answer: string; source: "gemini" | "fallback"; intent: string; data: any }> {
    const contextData = this.buildAssistantContextData(reportData, budgetData);

    const ai = await this.getGeminiClient();
    if (!ai && options.requireGemini) {
      throw new Error("Gemini API ključ nije podešen. Postavite GEMINI_API_KEY u backend .env fajlu.");
    }
...
`;
        ...
        if (options.requireGemini) {
          throw error instanceof Error ? error : new Error("Gemini poziv nije uspio.");
        }
      }
    }
